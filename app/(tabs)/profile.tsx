...
export default function ProfileScreen() {
  const [userData, setUserData] = useState({
    img_profile: require('@/assets/images/user.png'),
    username: '',
    email: '',
    phone: '',
    firstName: '',
        ...
        if (userId) {
          ...
          if (response.ok) {
            ...
            setUserData({
              ...
              profilePicture: userDataFromApi.img_profile ? { uri: UPLOAD_IMAGE_URL+userDataFromApi.img_profile } : require('@/assets/images/user.png'),
            });
          } else {
  ...
  const uploadProfileImage = async (imageUri, pkUser) => {
    ...
    try {
      ...
      setUserData(prevData => ({ ...prevData, profilePicture: require('@/assets/images/user.png') }));
      Alert.alert('Error', 'Hubo un problema de conexión al subir la imagen.');
    } finally {
      setIsLoading(false); 
    }
  };
